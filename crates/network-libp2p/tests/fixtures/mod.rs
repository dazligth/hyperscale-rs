//! Deterministic test fixtures for the libp2p transport tests.
//!
//! Provides seeded key generation and topology construction for the
//! validator-bind suite. That suite uses only a subset (key derivation +
//! validator key map), so the remaining helpers aren't dead code.
#![allow(dead_code)]
